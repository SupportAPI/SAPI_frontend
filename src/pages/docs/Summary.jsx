const Summary = ({ apiName, method, methodStyles, apiUrl, description, params, request, response }) => {
  ...
  return (
    <div className=''>
      ...
      <p className='text-2xl mt-4 text-[#666666] dark:text-dark-text'>{apiName || 'Enter API name'}</p>
      <div className='flex flex-row justify-content items-center'>
        ...
        <p className='my-2 ml-2 text-xl text-[#666666] dark:text-dark-text'>{apiUrl || 'No URL provided'}</p>
      </div>
      ...
      {/* Request Body */}
      <p className='text-xl text-[#666666] dark:text-dark-text mt-5 mb-2'>Request Body</p>
      {bodyType === 'NONE' ? (
        <p className='text-[#666666] dark:text-dark-text'>No Request Body</p>
      ) : bodyType === 'JSON' ? (
        <>
          <p className='text-lg text-[#666666] dark:text-dark-text mb-2'>Request Type : JSON </p>
          <div
        ...
        formData.map((formData, index) => (
          <>
            <p className='text-lg text-[#666666] dark:text-dark-text mb-2'>Request Type : Form-Data </p>
            <div key={index} className='flex flex-row text-[#666666] dark:text-dark-text'>
              <>
      ...
      {response?.length === 0 ? (
        ...
        statusCodes?.map((code, index) => (
          <>
            <p className='text-xl text-[#666666] dark:text-dark-text mt-5 mb-2'>{code}</p>
            <div
              className='text-[#666666] dark:text-dark-text bg-gray-100 p-2 rounded-md'
              style={{ whiteSpace: 'pre-wrap' }}
            >
